...
export class LocalSolarLLM {
  ...
  public generateResponse(query: string): LocalLLMResponse {
    ...
    if (this.isAboutCME(input)) {
      ...
      confidence = 0.8;
    } else if (this.isAboutSolarPhysics(input)) {
      response = this.generateSolarPhysicsResponse(input);
      confidence = 0.85;
  ...
  private isAboutSolarPhysics(input: string): boolean {
    return input.includes('solar physics') || input.includes('sun') || 
           input.includes('solar') || input.includes('photosphere') ||
           input.includes('chromosphere') || input.includes('corona') ||
           input.includes('sunspot') || input.includes('magnetic field');
  }
  ...
  private generateSolarPhysicsResponse(input: string): string {
    return `Solar Physics Overview

The Sun is a G-type main-sequence star at the center of our solar system. Here's what makes it fascinating:

Structure (Inside to Out):
- Core (15 million C): Nuclear fusion converts hydrogen to helium, releasing enormous energy
- Radiative Zone: Energy transfers outward via radiation over 170,000 years
- Convective Zone: Hot plasma rises, cool plasma sinks in convection cells
- Photosphere (5,500 C): The visible surface where sunspots appear
- Chromosphere: Reddish layer visible during eclipses
- Corona (1-3 million C): The outer atmosphere, source of solar wind

Key Solar Phenomena:
- Sunspots: Cooler, darker regions caused by intense magnetic activity
- Solar Flares: Sudden energy releases classified A to B to C to M to X
- CMEs: Billion-ton plasma eruptions traveling up to 3,000 km/s
- Solar Wind: Continuous stream of charged particles (400-800 km/s)

Solar Cycle:
The Sun follows an 11-year cycle of magnetic activity. We're currently in Solar Cycle 25, with increased CME and flare activity expected through 2025.
